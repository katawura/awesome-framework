...
if ( ! function_exists( 'awesome_setup' ) ) {
	function awesome_setup() {
		...
		load_theme_textdomain( 'awesome', $lang_dir );
		...
		register_nav_menus(
			array(
				'main-menu' => __( 'Main Menu', 'awesome' )
			)
		);
	}
 ...
 * ----------------------------------------------------------------------------------------
 */
if ( ! function_exists( 'awesome_post_meta' ) ) {
	function awesome_post_meta() {
		...
		if ( get_post_type() === 'post' ) {
			...
			if ( is_sticky() ) {
				echo '<li class="meta-featured-post"><i class="fa fa-thumb-tack"></i> ' . __( 'Sticky', 'awesome' ) . ' </li>';
			}
			...
			if ( comments_open() ) :
				...
				comments_popup_link( __( 'Leave a comment', 'awesome' ), __( 'One comment so far', 'awesome' ), __( 'View all % comments', 'awesome' ) );
				echo '</span>';
				echo '</li>';
			endif;
			...
			if ( is_user_logged_in() ) {
				echo '<li>';
				edit_post_link( __( 'Edit', 'awesome' ), '<span class="meta-edit">', '</span>' );
				echo '</li>';
			}
		}
	}
 ...
 * ----------------------------------------------------------------------------------------
 */
if ( ! function_exists( 'awesome_paging_nav' ) ) {
	function awesome_paging_nav() { ?>
		<ul>
			<?php 
 ...
 * ----------------------------------------------------------------------------------------
 */
if ( ! function_exists( 'awesome_widget_init' ) ) {
	function awesome_widget_init() {
		if ( function_exists( 'register_sidebar' ) ) {
			register_sidebar(
				array(
					'name' => __( 'Main Widget Area', 'awesome' ),
					'id' => 'sidebar-1',
					'description' => __( 'Appears on posts and pages.', 'awesome' ),
			...
			register_sidebar(
				array(
					...
					'id' => 'sidebar-2',
					'description' => __( 'Appears on the footer.', 'awesome' ),
 ...
 * ----------------------------------------------------------------------------------------
 */
if ( ! function_exists( 'awesome_validate_length' ) ) {
	function awesome_validate_length( $fieldValue, $minLength ) {
 ...
 * ----------------------------------------------------------------------------------------
 */
if ( ! function_exists( 'awesome_load_wp_head' ) ) {
	function awesome_load_wp_head() {
		// Get the logos
	...
	add_action( 'wp_head', 'awesome_load_wp_head' );
}
 ...
 * ----------------------------------------------------------------------------------------
 */
if ( ! function_exists( 'awesome_scripts' ) ) {
	function awesome_scripts() {
		...
		wp_register_script( 'awesome-custom', SCRIPTS . '/scripts.js', array( 'jquery' ), false, true );

		// Load the custom scripts
		wp_enqueue_script( 'bootstrap-js' );
		wp_enqueue_script( 'awesome-custom' );

		// Load the stylesheets
		wp_enqueue_style( 'font-awesome', THEMEROOT . '/css/font-awesome.min.css' );
		wp_enqueue_style( 'awesome-master', THEMEROOT . '/css/master.css' );
	}
